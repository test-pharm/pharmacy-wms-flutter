...
class MaterialModel {
  ...
  String get storageLocation => supplier;
  ...
  bool get isFullyExpired {
    if (batches.isNotEmpty) {
      ...
      if (activeBatches.isEmpty) return false;
      return activeBatches.every((b) => b.isExpired);
    }
  ...
  factory MaterialModel.fromJson(Map<String, dynamic> json) {
    ...
    final rawCategory =
        (json['categoryName'] ?? json['category'] ?? json['cat'] ?? '')
            .toString()
            .trim();
    final minStockLevel = _toInt(
      json['minStockLevel'] ?? json['min_stock_level'] ?? json['MinStockLevel'] ?? 0,
    );
    ...
    return MaterialModel(
      ...
      supplier: (json['supplier'] ?? json['supplierName'] ?? '').toString(),
