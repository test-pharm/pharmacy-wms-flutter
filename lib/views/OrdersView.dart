...
class _OrdersPageState extends State<OrdersPage> {  final TextEditingController _searchCtrl = TextEditingController();  String _selectedDateFilter = 'Filter by Date';  String _selectedStatusFilter = 'Filter by Status';  List<OrderModel> _orders = [];  Timer? _debounce;  bool _loadingOrders = true;  @override  void initState() {    super.initState();    _loadOrders();    OrderService.changes.addListener(_loadOrders);    NotificationService.changes.addListener(_handleNotificationChange);  }
  ...
  Widget _typeBadge(OrderType type) {    final (label, color) = switch (type) {      OrderType.add => (context.tr.orderTypeAdd, Colors.green),      OrderType.export => (context.tr.orderTypeExport, Colors.blue),      OrderType.edit => (context.tr.orderTypeEdit, Colors.orange),
      OrderType.refund => (context.tr.orderTypeRefund, Colors.purple),    };    return _badge(label, color);  }
  ...
  void _viewOrderDetails(OrderModel order, bool isDark) {    final expiry = order.expiryDate ?? (order.notes != null && !order.notes!.startsWith('Invoice: ') ? order.notes : null);    final invNum = order.invoiceNumber ?? (order.notes?.startsWith('Invoice: ') == true ? order.notes!.substring(9) : null);    showDialog(      context: context,      builder: (ctx) => AlertDialog(        title: Text(context.tr.ordersTitle),        content: SizedBox(          width: 520,          child: Column(            mainAxisSize: MainAxisSize.min,            children: [                            _detailRow(context.tr.orderId, order.id),
              if (invNum != null) _detailRow(context.tr.invoiceHash, invNum),              _detailRow(context.tr.product, order.productName),              _detailRow(context.tr.sku, order.productSku),              _detailRow(context.tr.quantity, '${order.quantity} ${order.unit}'),              _detailRow(context.tr.logNumber, order.logNumber),              _detailRow(context.tr.categoryId, order.categoryId.toString()),              _detailRow(context.tr.orderType, _typeLabel(order.type)),              _detailRow(context.tr.orderStatus, _statusLabel(order.status)),              _detailRow(context.tr.createdBy, order.createdBy),              _detailRow(context.tr.date, _formatDateTime(order.createdAt)),              if (expiry != null)                _detailRow(context.tr.expiryDate, _formatRawDate(expiry)),            ],          ),        ),        actions: [          TextButton(            onPressed: () => Navigator.pop(ctx),            child: Text(context.tr.close),          ),        ],      ),    );  }
  ...
  String _typeLabel(OrderType type) => switch (type) {    OrderType.add => context.tr.orderTypeAdd,    OrderType.export => context.tr.orderTypeExport,    OrderType.edit => context.tr.orderTypeEdit,
    OrderType.refund => context.tr.orderTypeRefund,  };  String _statusLabel(OrderStatus status) => switch (status) {    OrderStatus.completed => context.tr.orderStatusCompleted,    OrderStatus.pending => context.tr.orderStatusPending,    OrderStatus.canceled => context.tr.orderStatusCanceled,  };  String _formatDateTime(DateTime value) {    final local = value.toLocal();    final month = local.month.toString().padLeft(2, '0');    final day = local.day.toString().padLeft(2, '0');    final hour = local.hour.toString().padLeft(2, '0');    final minute = local.minute.toString().padLeft(2, '0');    return '${local.year}-$month-$day $hour:$minute';  }
...
class _OrderCardWidgetState extends State<OrderCardWidget> {
  ...
  Widget _typeBadge(OrderType type, AppLocalizations tr) {
    final (label, color) = switch (type) {
      ...
      OrderType.refund => (tr.orderTypeRefund, Colors.purple),
    };
